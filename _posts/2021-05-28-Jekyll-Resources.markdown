...
Optional info: the head.html has a tag. This tag should extract the above code block and render the favicon. However, in my case it didn't work that simply. 
```` 
             {%- include custom-head.html -%}
             
```` 

I tried to create a head.html file in my *_includes* directory but then my CCS vanished, because head.html searches for a css file and then css file import other files. To avoid all that, I decided to move with below step. 
